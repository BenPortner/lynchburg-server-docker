...
def websocket_send(url, message, hmac_key=None, group='default'):
    sig = hmac_key and hmac.new(to_bytes(hmac_key), to_bytes(message), digestmod=hashlib.md5).hexdigest() or ''
    params = urlencode(
    ...
    def post(self):
        ...
        if 'message' in self.request.arguments:
            ...
            if hmac_key:
                ...
                actual_signature = hmac.new(to_bytes(hmac_key), to_bytes(message), digestmod=hashlib.md5).hexdigest()
